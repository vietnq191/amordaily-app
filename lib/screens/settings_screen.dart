...
class SettingsScreen extends StatelessWidget {
  ...
  Widget _divider() => Divider(height: 1, color: Colors.white.withValues(alpha: 0.07), indent: 16, endIndent: 16);

  Widget _editTile(BuildContext context, {required String title, required String value, required VoidCallback onTap}) =>
      ListTile(
  ...
  Widget _dateTile(BuildContext context, {required String title, required dynamic date, required VoidCallback onTap}) {
    ...
    return ListTile(
      title: Text(title, style: const TextStyle(color: Colors.white70, fontSize: 13)),
      subtitle: Text(sub, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
      trailing: const Icon(Icons.calendar_today_rounded, color: Colors.white30, size: 18),
      onTap: onTap,
    );
  }

  Widget _imageTile({required String title, String? path, required VoidCallback onTap}) =>
      ListTile(
        title: Text(title, style: const TextStyle(color: Colors.white70, fontSize: 13)),
        trailing: path != null
  ...
  Future<void> _editName(BuildContext context, LoveProvider provider, bool isP1, AppLocalizations loc) async {
    final ctrl = TextEditingController(text: isP1 ? provider.story.partner1Name : provider.story.partner2Name);
    await showDialog(
      context: context,
  ...
  Future<void> _selectDate(BuildContext context, LoveProvider provider, bool isP1, bool isBirthDate, AppLocalizations loc) async {
    final initial = isBirthDate
        ? (isP1 ? provider.story.partner1BirthDate : provider.story.partner2BirthDate) ?? DateTime(2000)
  ...
  Future<void> _pickImage(BuildContext context, LoveProvider provider, bool isP1) async {
  ...
  Future<void> _showLanguageSelectDialog(BuildContext context, LoveProvider provider, AppLocalizations loc) async {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)),
