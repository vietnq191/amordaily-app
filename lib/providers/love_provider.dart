...
class LoveStory {
  ...
  LoveStory({
    ...
    List<Milestone>? customMilestones,
        ...
        partner2Name = partner2Name ?? (language == 'vi' ? 'Người ấy' : 'Partner'),
  ...
  factory LoveStory.fromJson(Map<String, dynamic> json) {
    ...
    return LoveStory(
      partner1Name: json['partner1Name'] ?? (json['language'] == 'vi' ? 'Bạn' : 'You'),
      partner2Name: json['partner2Name'] ?? (json['language'] == 'vi' ? 'Người ấy' : 'Partner'),
      ...
      customMilestones: milestonesJson.map((m) => Milestone.fromJson(m as Map<String, dynamic>)).toList(),
    );
  }
}
...
class LoveProvider with ChangeNotifier {
  ...
  String getZodiacSign(DateTime? birthDate, {String? lang}) {
    ...
    final Map<String, List<String>> zodiacNames = {
      'vi': ['Bảo Bình', 'Song Ngư', 'Bạch Dương', 'Kim Ngưu', 'Song Tử', 'Cự Giải', 'Sư Tử', 'Xử Nữ', 'Thiên Bình', 'Bọ Cạp', 'Nhân Mã', 'Ma Kết'],
      ...
      'ko': ['물병자리', '물고기자리', '양자리', '황소자리', '쌍둥이자리', '게자리', '사자자리', '처녀자리', '천칭자리', '전갈자리', '사수자리', '염소자리'],
    ...
    int idx;
    if ((month == 1 && day >= 20) || (month == 2 && day <= 18)) idx = 0;
    else if ((month == 2 && day >= 19) || (month == 3 && day <= 20)) idx = 1;
    else if ((month == 3 && day >= 21) || (month == 4 && day <= 19)) idx = 2;
    else if ((month == 4 && day >= 20) || (month == 5 && day <= 20)) idx = 3;
    else if ((month == 5 && day >= 21) || (month == 6 && day <= 20)) idx = 4;
    else if ((month == 6 && day >= 21) || (month == 7 && day <= 22)) idx = 5;
    else if ((month == 7 && day >= 23) || (month == 8 && day <= 22)) idx = 6;
    else if ((month == 8 && day >= 23) || (month == 9 && day <= 22)) idx = 7;
    else if ((month == 9 && day >= 23) || (month == 10 && day <= 22)) idx = 8;
    else if ((month == 10 && day >= 23) || (month == 11 && day <= 21)) idx = 9;
    else if ((month == 11 && day >= 22) || (month == 12 && day <= 21)) idx = 10;
    else idx = 11;
  ...
  List<Map<String, dynamic>> getFilteredMilestones({bool includeAllCustom = false}) {
    ...
    for (int i = 1; i <= 50; i++) {
      ...
      if (d.isAfter(now)) {
        all.add({'title': '$i ${loc.t('years_together')}', 'date': d, 'isDefault': true});
        yearlyCount++;
    ...
    for (int i = 0; i < _story.customMilestones.length; i++) {
      ...
      if (includeAllCustom || (diff > 0 && diff <= 60)) {
        all.add({'title': m.title, 'date': m.date, 'isDefault': false, 'customIndex': i});
      }
    }

    /* 3. Birthdays (Always show if within 60 days) */
    if (_story.partner1BirthDate != null) {
      DateTime bd1 = DateTime(now.year, _story.partner1BirthDate!.month, _story.partner1BirthDate!.day);
      if (bd1.isBefore(now)) bd1 = DateTime(now.year + 1, bd1.month, bd1.day);
      if (includeAllCustom || bd1.difference(now).inDays <= 60) {
        all.add({'title': loc.t('your_birthday_event'), 'date': bd1, 'isDefault': false});
      }
    }
    if (_story.partner2BirthDate != null) {
      DateTime bd2 = DateTime(now.year, _story.partner2BirthDate!.month, _story.partner2BirthDate!.day);
      if (bd2.isBefore(now)) bd2 = DateTime(now.year + 1, bd2.month, bd2.day);
      if (includeAllCustom || bd2.difference(now).inDays <= 60) {
        all.add({'title': loc.t('partner_birthday_event'), 'date': bd2, 'isDefault': false});
      }
    }

    all.sort((a, b) => (a['date'] as DateTime).compareTo(b['date'] as DateTime));
    return all;
  }
}
